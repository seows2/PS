...
function solution(n, costs) {
    ...
    const updateTable = (from, to, tables) => {
        const fromTable = table[from]
        const toTabel = table[to]
        tables.forEach((table, index) => {
            if(table === fromTable) tables.splice(index, 1, toTabel)
        })
        return tables
    }
    ...
    let table = Array(n).fill(0).map((_,index) => index)

    for (const cost of costs) {
        if(condition(table)) break
        const [from, to, value] = cost
        //console.log(from, to, value, table);
        if(table[from] !== table[to]){
            table = updateTable(from, to, table)
            ans+=value
        }
    }
    return ans
