...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    const applesPos = apples.map(apple => apple + a)
    const orangesPos = oranges.map(orange => orange + b)
    let appleCnt = 0;
    let orangeCnt = 0;
    for(const apple of applesPos){
        if(apple < s || apple > t) continue;
        appleCnt++;
    }
    for(const orange of orangesPos){
        if(orange < s || orange > t) continue;
        orangeCnt++;
    }
