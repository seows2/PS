...
자연수가 담긴 배열이 주어질 때, 애너그램 관계인 숫자들끼리 묶어서 그룹을 만들려합니다.
예를 들어 배열에 담긴 숫자가 [112, 1814, 121, 1481, 1184]이라면 [112, 121]이 한 그룹이 되며,
[1814, 1481, 1184]가 한 그룹이 됩니다. 각 숫자들은 같은 그룹에 속하는 다른 모든 숫자들과 애너그램 관계입니다.
...
function solution(arr) {
  ...
  function sortWord(word) {
    return String(word).split("").sort().join("");
  }
  arr.forEach((e) => {
    const sortedWord = sortWord(e);
    ans.push(sortedWord);
  });
  ans = ans.filter((item, index) => ans.indexOf(item) === index);
  return ans.length;
}
